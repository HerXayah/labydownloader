...
export async function downloadSpecificMC(channel) {
  return await makeHttpRequest(
    `https://laby-releases.s3.de.io.cloud.ovh.net/api/v1/manifest/${channel}/latest.json`
  );
}
...
export async function downloadAssets(
  ...
  return await new Promise((resolve, reject) => {
    https
      .get(
        `https://releases.r2.labymod.net/api/v1/download/assets/labymod4/${channel}/${commitReference}/${name}/${checksum}.jar`,
        (response) => {
          if (response.statusCode === 200) {
            const file = fs.createWriteStream(targetFile);
            response.pipe(file);
...
export async function downloadAssetsGUI(
  channel,
  commitReference,
  name,
  checksum,
